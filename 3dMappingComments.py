...
FORMAT FOR RUNNING CODE:

python script.py --recordingFolder 
/path/to/recording/folder --outputFolder 
/path/to/output/folder --trajectory.

- As car moves around object, mapping gets updated
- Camera pos and orientation visualized in Open3D
...
# Function to blend two colors based on a parameter 'm'
def blend(a, b, m):
    return [a[0] * (1 - m) + b[0] * m, a[1] * (1 - m) + b[1] * m, a[2] * (1 - m) + b[2] * m]

# Class for storing camera trajectory
class Trajectory:
    def __init__(self):
        self.maxPoints = 1000
        self.points = []
...
# Class for managing a single point cloud
class PointCloud:
...
# Class for representing the camera frame
class CoordinateFrame:
    def __init__(self, scale=0.25):
        # Create a coordinate frame mesh using Open3D
...
# Class for visualizing the point clouds and camera frame
...
if __name__ == '__main__':
    ...
    def onMappingOutput(output):
        for frameId in output.updatedKeyFrames:
            ...
            # Remove deleted key frames from visualization
            if not keyFrame:
